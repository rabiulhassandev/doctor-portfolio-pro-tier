    ...
    <div class="mx-auto max-w-7xl px-5 py-14 sm:px-8 sm:py-20">
        <div class="grid gap-10 lg:grid-cols-[1fr_1.1fr] lg:gap-12">
            ...
            <div class="flex flex-col gap-8" data-reveal>
                ...
                {{-- The ways to make contact. One divided block rather than
                     three separate cards: on a phone these are the whole page,
                     and a single list of full-width rows is both easier to hit
                     and quieter than a wall of panels. --}}
                <div class="flex flex-col divide-y divide-line border-y border-line">
                    @if ($doctor->telHref())
                        <a href="{{ $doctor->telHref() }}"
                           class="row-editorial group/link flex items-center gap-4 px-3 py-4 focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-brass">
                            <span class="flex size-10 shrink-0 items-center justify-center border border-line text-muted transition-colors duration-400 group-hover/link:border-brass group-hover/link:text-brass" aria-hidden="true">
                            ...
                            <span class="flex min-w-0 flex-col">
                                <span class="eyebrow">Telephone</span>
                    ...
                    @if ($whatsapp = $doctor->whatsappHref())
                        <a href="{{ $whatsapp }}" target="_blank" rel="noopener noreferrer"
                           class="row-editorial group/link flex items-center gap-4 px-3 py-4 focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-brass">
                            {{-- WhatsApp keeps its own green. It is the one
                                 colour on the site that is not ours to choose:
                                 people scan for that mark, and recolouring it in
                                 brass makes it stop being a WhatsApp button. --}}
                            <span class="flex size-10 shrink-0 items-center justify-center border border-line text-[#128C7E] transition-colors duration-400 group-hover/link:border-[#25D366]" aria-hidden="true">
                ...
                @if ($address = $doctor->fullAddress())
                    <div class="flex flex-col gap-2">
                        <div class="flex items-center gap-3">
                            ...
                            <h2 class="eyebrow">{{ $doctor->chamber_name ?: 'The chamber' }}</h2>
                        </div>
                ...
                <div class="flex flex-col gap-4">
                    <div class="flex flex-wrap items-center justify-between gap-3">
                        <div class="flex items-center gap-3">
                            <span class="rule-brass" aria-hidden="true"></span>
                            <h2 class="eyebrow">Opening hours</h2>
                        </div>
            ...
            <div class="flex flex-col gap-4" data-reveal="100">
                @if ($mapUrl)
                    <div class="overflow-hidden border border-line shadow-card">
                        <iframe src="{{ $mapUrl }}"
                    ...
                    @endif
                @else
                    <div class="flex aspect-[4/3] items-center justify-center border border-dashed border-line-strong bg-paper-shade text-center text-muted lg:h-full">
