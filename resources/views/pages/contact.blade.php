                ...
                <div class="grid gap-3 sm:grid-cols-2">
                    @if ($doctor->telHref())
                        <a href="{{ $doctor->telHref() }}"
                           class="card-lift flex items-center gap-3 rounded-2xl border border-line bg-surface p-5 shadow-card hover:border-brand hover:shadow-lift">
                    ...
                    @if ($whatsapp = $doctor->whatsappHref())
                        <a href="{{ $whatsapp }}" target="_blank" rel="noopener noreferrer"
                           class="card-lift flex items-center gap-3 rounded-2xl border border-line bg-surface p-5 shadow-card hover:border-brand hover:shadow-lift">
                            <span class="flex size-11 shrink-0 items-center justify-center rounded-xl bg-[#25D366]/12 text-[#128C7E]" aria-hidden="true">
                    ...
                    @if ($doctor->email)
                        <a href="mailto:{{ $doctor->email }}"
                           class="card-lift flex items-center gap-3 rounded-2xl border border-line bg-surface p-5 shadow-card hover:border-brand hover:shadow-lift sm:col-span-2">
                            <span class="flex size-11 shrink-0 items-center justify-center rounded-xl bg-accent-soft text-accent" aria-hidden="true">
                                <svg class="size-5" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            @foreach ($hours as $row)
                                <div @class([
                                    ...
                                    <dt>
                                        {{ $row['label'] }}
                                        @if ($row['is_today'])
                                            <span class="ml-1 text-xs font-normal text-accent">Today</span>
                                        @endif
                                    </dt>
                                    <dd>
                ...
                @if ($mapUrl)
                    ...
                    @if ($doctor->map_latitude && $doctor->map_longitude)
                        <a href="https://www.google.com/maps/dir/?api=1&destination={{ $doctor->map_latitude }},{{ $doctor->map_longitude }}"
                           ...
                           class="link-underline w-fit font-semibold text-brand">
                            Get directions
                        </a>
                    @endif
                @else
                    <div class="flex aspect-[4/3] items-center justify-center rounded-2xl border border-dashed border-line-strong bg-paper-shade text-center text-muted lg:h-full">
