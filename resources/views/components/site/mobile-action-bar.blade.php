...
@if ($tel || $bookingEnabled)
    <div class="fixed inset-x-0 bottom-0 z-40 border-t border-line bg-paper/95 backdrop-blur-md lg:hidden"
        ...
        <div class="flex items-stretch gap-2 px-3 py-2.5">
            @if ($tel)
                <a href="{{ $tel }}"
                   class="flex flex-1 items-center justify-center gap-2 rounded-full border border-line-strong bg-surface px-4 py-3 text-sm font-semibold text-ink transition-colors active:bg-paper-shade">
            ...
            @if ($bookingEnabled)
                <a href="{{ route('booking') }}"
                   class="flex flex-1 items-center justify-center gap-2 rounded-full bg-brand px-4 py-3 text-sm font-semibold text-white shadow-card transition-colors active:bg-brand-dark">
                    <svg class="size-4.5" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor" aria-hidden="true">
                        <path stroke-linecap="round" stroke-linejoin="round"
